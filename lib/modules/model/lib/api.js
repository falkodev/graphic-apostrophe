        ...
        if (schemaField.showFields) {
          schemaField.showFields.forEach(showField => {
            if (field[schema.name].includes(schemaField.value)) {
              ...
              if (field.type === 'area') {
                if (showField === 'areaOptions') {
                  acc = Object.assign({}, acc, {
                    options: {
                      widgets: extractedOptions,
                    },
                  })
                } else if (showField === 'contextual') {
                  acc = Object.assign({}, acc, { contextual: extractedOptions })
                }
              } else {
                if (field.type === 'slug') {
                  acc[schemaField.value] = { enableSlug: extractedOptions }
                } else {

...
module.exports = {
  ...
  beforeConstruct: async (self, options) => {
    ...
    try {
      ...
      const folders = await readDirAsync(self.defaultPath)
      const widgetNames = folders.filter(folder => folder.endsWith('-widgets'))
      for (const widgetName of widgetNames) {
        const file = await readFileAsync(`${self.defaultPath}/${widgetName}/index.js`, 'utf-8')
  ...
  construct(self, options) {
    ...
    self.afterSave = async (req, piece, options, callback) => {
      ...
      try {
        await mkdirAsync(`${self.defaultPath}/${piece.slug}`)
      } catch (error) {
      ...
      try {
        ...
        await writeFileAsync(`${self.defaultPath}/${piece.slug}/index.js`, module, encoding)
